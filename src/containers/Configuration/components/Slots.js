...
const Slots = (props) => {
    ...
    const addSlot = () => {
        setInputFields([...inputFields, { id: uuidv4(), fromTime: '', toTime: '', date: '', location: '' , romTime1: '', toTime1: '',}])
    ...
    const handleSubmit = async () => {
        try {
            ...
            setShow(false)
            seterror('');
            console.log('jj', inputFields)
            dispatch(createSlot(inputFields))
            setInputFields([{ id: uuidv4(), fromTime: '', toTime: '', date: '', location: '' , romTime1: '', toTime1: '',}])
        } catch (error) {
            console.error(error)
        }
    ...
    const handleChangeTimeInput = (id, event, name) => {
        const newInputFields = inputFields.map(i => {
            if (id === i.id) {
                let t= event.target.value
                let [h,m] = t.split(":");
                let T = (h%12+12*(h%12==0))+":"+m
                let AP = h >= 12 ? 'PM' : 'AM'
                let va = T + ' ' + AP
                console.log('t', va)
                i[event.target.name] = va
